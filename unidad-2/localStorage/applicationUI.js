export class ApplicationUI
{
    ...
    editUserUI()
    {
        ...
        let user = this.model.isValidUserGetData(username);
        if (!user)
        {
            this.showAlert("Usuario no encontrado.");
            return;
        ...
        let newUsername = this.getPrompt("Nuevo nombre de usuario:", user.name);
        if (newUsername === null)
        {
        ...
        let newName = this.getPrompt("Nuevo nombre completo:", user.full_name);
