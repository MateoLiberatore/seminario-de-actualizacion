...
void client_menu(){
    ...
    while (true)
    {
        ...
        if (option == "1")                       { buy_product(); }
        else if (option == "2")                  { password_mod(); }
        else if (option == "x" || option == "X") { return; }
        else {std::cout << "Opcion invalida, intentelo nuevamente" << std::endl;}
    }
}
...
void add_user() {
    ...
    std::string new_name, new_pass, category = "Cliente";
    sesion.user_count++;
    std::cout << "Crear Usuario:" << std::endl;
    std::cout << "Nombre de usuario: "; std::cin >> new_name;

    for (int i = 0; i < total; i++) {
        if (users[i].user_name == new_name) {
            std::cout << "Ese nombre de usuario ya existe. Intente con otro." << std::endl;
            return;
        }
    }
    ...
    std::cout << "No se pudo crear el usuario. Límite alcanzado." << std::endl;
    sesion.user_count--;
}
...
int main(){
    ...
    else { std::cout << "Usuario registrado sin categoria asignada o, contacte un adminitrador" << std::endl; 
    system("pause"); 
    break;}
    }
